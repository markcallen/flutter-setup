...
def get_metric_statistics(
    ...
    if dimensions:
        args.extend(
            [
                "--dimensions",
                *[f"Name={name},Value={value}" for name, value in dimensions],
            ]
        )
...
def latest_metric_stat(
    ...
    points = get_metric_statistics(
        profile, region, namespace, metric_name, dimensions, start, end, period, [stat]
    )
    if not points:
        return None
...
def sum_metric_over_hours(
    ...
    start = end - dt.timedelta(hours=hours)
    points = get_metric_statistics(
        profile, region, namespace, metric_name, dimensions, start, end, period, ["Sum"]
    )
    if not points:
        return None
...
def wait_for_logs_query(
    profile: str, region: str, query_id: str, timeout_seconds: int = 60
) -> list[dict[str, str]]:
    ...
    while time.time() < deadline:
        ...
        if status == "Complete":
            ...
            for row in data.get("results", []):
                results.append(
                    {item.get("field", ""): item.get("value", "") for item in row}
                )
            return results
        ...
        time.sleep(2)
    raise AwsCommandError(
        f"Timed out waiting for CloudWatch Logs Insights query {query_id}"
    )
...
def check_cloudwatch_alarms(profile: str, region: str) -> dict[str, Any]:
    ...
    if data is None:
        return make_check(
            ...
            "ERROR",
            [make_finding("INFO", "Could not retrieve alarm state.", "", "None")],
            ["api_error=true"],
        )
    ...
    for alarm in metric_alarms:
        ...
        evidence.append(
            f"ALARM metric={namespace}/{metric} alarm={name} actions={'yes' if actions else 'NONE'}"
        )
    ...
    if not alarms_in_alarm:
        return make_check(
            ...
            "PASS",
            [],
            evidence or ["no_actionable_alarms_firing=true"],
        )
...
def check_aws_health_events(profile: str, region: str) -> dict[str, Any]:
    ...
    if not events:
        return make_check(
            ...
            "PASS",
            [],
            ["no_health_events_returned=true"],
        )
    ...
    for event in events:
        ...
        evidence.append(
            f"event service={service} type={event_type} status={event_status} scope={scope} start={start_label}"
        )
    ...
    if not actionable_arns:
        return make_check(
            "aws-health-events", "AWS Health account events", "PASS", [], evidence[:20]
        )

    details = run_aws_json_safe(
        profile,
        region,
        ["health", "describe-event-details", "--event-arns", *actionable_arns[:10]],
    )
    entities = run_aws_json_safe(
        profile,
        region,
        [
            "health",
            ...
            "--filter",
            json.dumps({"eventArns": actionable_arns[:10]}),
        ],
    )
    entities_by_event: dict[str, list[str]] = {}
    if entities is not None:
        for item in entities.get("entities", []):
            entities_by_event.setdefault(item.get("eventArn", ""), []).append(
                item.get("entityValue", "?")
            )
    ...
    if not detailed_events:
        detailed_events = [
            ...
            for event in events
            if event.get("arn") in actionable_arns
        ]
    ...
    for detail in detailed_events:
        ...
        status = (
            "FAIL" if severity == "HIGH" else ("WARN" if status == "PASS" else status)
        )
        findings.append(
            make_finding(
                severity=severity,
...
def check_log_group_retention(profile: str, region: str) -> dict[str, Any]:
    ...
    if not no_retention:
        return make_check(
            ...
            "PASS",
            [],
            evidence or ["all_groups_have_retention=true"],
        )
...
def check_alb_access_logging(profile: str, region: str) -> dict[str, Any]:
    ...
    for lb in lbs_data.get("LoadBalancers", []):
        ...
        evidence.append(
            f"lb={name} access_log_enabled={logging_enabled} bucket={log_bucket or 'none'}"
        )
...
def check_alb_service_signals(profile: str, region: str) -> dict[str, Any]:
    ...
    for lb in lbs_data.get("LoadBalancers", []):
        ...
        elb_5xx = sum_metric_over_hours(
            profile, region, "AWS/ApplicationELB", "HTTPCode_ELB_5XX_Count", dims, 24
        )
        target_5xx = sum_metric_over_hours(
            profile, region, "AWS/ApplicationELB", "HTTPCode_Target_5XX_Count", dims, 24
        )
        resp_avg = avg_metric_over_hours(
            profile, region, "AWS/ApplicationELB", "TargetResponseTime", dims, 24
        )
        resp_max = max_metric_over_hours(
            profile, region, "AWS/ApplicationELB", "TargetResponseTime", dims, 24
        )
    ...
    return make_check(
        "alb-service-signals", "ALB 5XX and latency signals", status, findings, evidence
    )
...
def _get_rds_metric(
    profile: str, region: str, db_id: str, metric_name: str, stat: str = "Average"
) -> float | None:
    end = utc_now()
...
def check_rds_health(profile: str, region: str) -> dict[str, Any]:
    instances_data = run_aws_json_safe(
        profile, region, ["rds", "describe-db-instances"]
    )
    if instances_data is None:
        return make_check(
            "rds-health",
            "RDS instance health and performance",
            "ERROR",
            [],
            ["api_error=true"],
        )
    ...
    for instance in instances_data.get("DBInstances", []):
        ...
        if free_bytes is not None:
            ...
            used_pct = (
                ((allocated_bytes - free_bytes) / allocated_bytes) * 100
                if allocated_bytes > 0
                else 0
            )
            free_gb = free_bytes / 1024**3
            evidence.append(
                f"rds={db_id} storageFreeGB={free_gb:.1f} storageUsedPct={used_pct:.1f}%"
            )
    ...
    return make_check(
        "rds-health", "RDS instance health and performance", status, findings, evidence
    )
...
def check_rds_deep_metrics(profile: str, region: str) -> dict[str, Any]:
    ...
    if instances_data is None:
        return make_check(
            "rds-deep-metrics",
            "RDS memory, connections, and latency",
            "ERROR",
            [],
            ["api_error=true"],
        )
    ...
    for instance in instances_data.get("DBInstances", []):
        ...
        freeable_bytes = latest_metric_stat(
            profile, region, "AWS/RDS", "FreeableMemory", dims, 1, "Average"
        )
        ...
        connections = latest_metric_stat(
            profile, region, "AWS/RDS", "DatabaseConnections", dims, 1, "Average"
        )
        ...
        read_latency = latest_metric_stat(
            profile, region, "AWS/RDS", "ReadLatency", dims, 1, "Average"
        )
        write_latency = latest_metric_stat(
            profile, region, "AWS/RDS", "WriteLatency", dims, 1, "Average"
        )
        ...
        queue_depth = latest_metric_stat(
            profile, region, "AWS/RDS", "DiskQueueDepth", dims, 1, "Average"
        )
...
def check_asg_health(profile: str, region: str) -> dict[str, Any]:
    asgs_data = run_aws_json_safe(
        profile, region, ["autoscaling", "describe-auto-scaling-groups"]
    )
    ...
    for asg in asgs_data.get("AutoScalingGroups", []):
        ...
        unhealthy_instances = [
            i for i in instances if i.get("HealthStatus") == "Unhealthy"
        ]
        ...
        if len(in_service) < desired:
            ...
            status = (
                "FAIL"
                if severity == "HIGH"
                else ("WARN" if status == "PASS" else status)
            )
            findings.append(
                make_finding(
                    severity=severity,
                ...
                if refresh_status in ("Failed", "Cancelled") and start_time_str:
                    try:
                        ...
                        if start_time > cutoff:
                            evidence.append(
                                f"asg={name} instance_refresh status={refresh_status} started={start_time_str}"
                            )
                            if status == "PASS":
                                status = "WARN"
                            findings.append(
    ...
    return make_check(
        "asg-health", "Auto Scaling Group health", status, findings, evidence
    )
...
def check_ec2_stopped_instances(profile: str, region: str) -> dict[str, Any]:
    ...
    for reservation in data.get("Reservations", []):
        for instance in reservation.get("Instances", []):
            ...
            evidence.append(
                f"stopped_instance={iid} type={itype} name={name_tag} launched={launch_time}"
            )
...
def check_ec2_runtime_health(profile: str, region: str) -> dict[str, Any]:
    ...
    for item in status_data.get("InstanceStatuses", []):
        ...
        evidence.append(
            f"instance={label} instanceStatus={instance_status} systemStatus={system_status} ebsStatus={ebs_status}"
        )
        ...
        if (
            ...
            or system_status != "ok"
            or ebs_status
            not in {"ok", "not-applicable", "initializing", "insufficient-data", "n/a"}
        ):
        ...
        for event in item.get("Events", []):
            ...
            evidence.append(
                f"scheduled_event instance={label} code={code} notBefore={not_before}"
            )
            if status == "PASS":
                status = "WARN"
            findings.append(
...
def check_apprunner_health(profile: str, region: str) -> dict[str, Any]:
    ...
    if degraded:
        ...
        return make_check(
            "apprunner-health", "App Runner service health", status, findings, evidence
        )

    return make_check(
        "apprunner-health", "App Runner service health", "PASS", [], evidence
    )
...
def _get_elasticache_metric(
    profile: str, region: str, cluster_id: str, metric_name: str
) -> float | None:
    end = utc_now()
    start = end - dt.timedelta(hours=1)
    data = run_aws_json_safe(
        ...
        [
            ...
            metric_name,
            "--dimensions",
            f"Name=CacheClusterId,Value={cluster_id}",
            "--start-time",
            start.strftime("%Y-%m-%dT%H:%M:%SZ"),
            "--end-time",
            end.strftime("%Y-%m-%dT%H:%M:%SZ"),
...
def check_elasticache_health(profile: str, region: str) -> dict[str, Any]:
    ...
    for cluster in clusters:
        ...
        evidence.append(
            f"cluster={cluster_id} status={cluster_status} engine={engine} {engine_version} type={node_type}"
        )
    ...
    return make_check(
        "elasticache-health", "ElastiCache cluster health", status, findings, evidence
    )
...
def check_alarms_no_actions(profile: str, region: str) -> dict[str, Any]:
    data = run_aws_json_safe(
        profile,
        region,
        ["cloudwatch", "describe-alarms", "--alarm-types", "MetricAlarm"],
    )
    if data is None:
        return make_check(
            "alarms-no-actions",
...
def check_billing_mtd_spend(profile: str, region: str) -> dict[str, Any]:
    ...
    def _get_top_services(
        start: dt.date, end: dt.date, top_n: int = 6
    ...
    if mtd is None:
        return make_check(
            "billing-mtd-spend",
            "Billing: month-to-date spend",
            "ERROR",
            [
                make_finding(
                    "INFO",
                    f"Could not retrieve Cost Explorer data. Ensure Cost Explorer is enabled and the AWS profile {profile} has ce:GetCostAndUsage permission.",
                    "",
                    "None",
                )
            ],
    ...
    if prior is not None:
        evidence.append(
            f"prior_same_period_spend=${prior:.2f} period={last_month_same_start}..{last_month_same_end}"
        )
        if prior > 0:
    ...
    if not findings:
        ...
        if prior is not None:
            ...
            findings_msg += (
                f" ({pct_change:+.1f}% vs same period last month ${prior:.2f})"
            )
        evidence.insert(0, findings_msg)

    return make_check(
        "billing-mtd-spend", "Billing: month-to-date spend", status, findings, evidence
    )
...
def check_billing_cost_anomalies(profile: str, region: str) -> dict[str, Any]:
    ...
    anomalies_data = run_aws_json_safe(
        profile,
        region,
        [
            "ce",
    ...
    evidence.append(
        f"open_anomalies={len(open_anomalies)} resolved={len(resolved_anomalies)}"
    )
    ...
    for anomaly in open_anomalies:
        ...
        evidence.append(
            f"open_anomaly start={start} totalImpact=${total_impact:.2f} maxImpact=${max_impact:.2f} root={root_desc}"
        )
    ...
    for anomaly in resolved_anomalies[:3]:
        ...
        evidence.append(
            f"resolved_anomaly start={start} end={end} totalImpact=${total_impact:.2f}"
        )
...
def check_billing_idle_resources(profile: str, region: str) -> dict[str, Any]:
    ...
    if ebs_data is not None:
        ...
        for v in volumes:
            ...
            evidence.append(
                f"unattached_ebs={vid} size={size}GB type={vtype} created={create_time} name={name_tag}"
            )
    ...
    if eip_data is not None:
        ...
        for eip in unassociated:
            ...
            evidence.append(
                f"unassociated_eip={pub_ip} alloc={alloc_id} name={name_tag}"
            )
    ...
    return make_check(
        "billing-idle-resources",
        "Billing: idle resource waste (EBS + EIPs)",
        status,
        findings,
        evidence,
    )
...
def check_backup_restore_readiness(profile: str, region: str) -> dict[str, Any]:
    ...
    protected_data = run_aws_json_safe(
        profile, region, ["backup", "list-protected-resources"]
    )
    backup_jobs_data = run_aws_json_safe(
        ...
        [
            ...
            "list-backup-jobs",
            "--by-created-after",
            iso_z(utc_now() - dt.timedelta(days=7)),
        ],
    )
    restore_jobs_data = run_aws_json_safe(
        ...
        [
            ...
            "list-restore-jobs",
            "--by-created-after",
            iso_z(utc_now() - dt.timedelta(days=90)),
        ],
    )
    ...
    if (
        plans_data is None
        or protected_data is None
        or backup_jobs_data is None
        or restore_jobs_data is None
        or rds_data is None
    ):
        return make_check(
    ...
    evidence.append(
        f"backup_plans={len(plans)} protected_resources={len(protected)} backup_jobs_last_7d={len(backup_jobs)} restore_jobs_last_90d={len(restore_jobs)}"
    )
    ...
    for db in rds_data.get("DBInstances", []):
        ...
        if db_arn:
            evidence.append(
                f"rds_backup_coverage db={db_id} protected={db_arn in protected_arns}"
            )
    ...
    completed_restore_jobs = [
        job for job in restore_jobs if job.get("Status") == "COMPLETED"
    ]
    if not completed_restore_jobs:
        ...
        latest_restore = max(
            (
                parse_aws_datetime(job.get("CreationDate"))
                for job in completed_restore_jobs
            ),
            default=None,
        )
        if latest_restore is not None:
            evidence.append(
                f"latest_completed_restore_test={latest_restore.strftime('%Y-%m-%d')}"
            )
...
def check_alarm_coverage(profile: str, region: str) -> dict[str, Any]:
    ...
    if (
        ...
        or lbs_data is None
        or rds_data is None
        or asgs_data is None
        or ec2_data is None
    ):
        return make_check(
            "alarm-coverage",
            "Alarm coverage for critical resources",
            "ERROR",
            [],
            ["api_error=true"],
        )
    ...
    alarms = [
        alarm
        for alarm in alarms_data.get("MetricAlarms", [])
        if not is_target_tracking_alarm_name(str(alarm.get("AlarmName", "")))
    ]
    findings: list[dict[str, Any]] = []
    evidence: list[str] = []
    status = "PASS"
    ...
    def has_alarm(
        metric_names: set[str], dimension_name: str, dimension_value: str
    ) -> bool:
        for alarm in alarms:
    ...
    for lb in lbs_data.get("LoadBalancers", []):
        ...
        evidence.append(
            f"alarm_coverage alb={name} missing={','.join(missing) or 'none'}"
        )
        if missing:
            if status == "PASS":
                status = "WARN"
    ...
    for db in rds_data.get("DBInstances", []):
        ...
        evidence.append(
            f"alarm_coverage rds={db_id} missing={','.join(missing) or 'none'}"
        )
        if missing:
            if status == "PASS":
                status = "WARN"
    ...
    for asg in asgs_data.get("AutoScalingGroups", []):
        name = asg.get("AutoScalingGroupName", "unknown")
        has_inservice = has_alarm(
            {"GroupInServiceInstances"}, "AutoScalingGroupName", name
        )
...
def check_certificate_expiry(profile: str, region: str) -> dict[str, Any]:
    ...
    return make_check(
        "certificate-expiry", "ACM certificate expiration", status, findings, evidence
    )
...
def check_slow_queries(profile: str, region: str) -> dict[str, Any]:
    log_groups_data = run_aws_json_safe(
        profile,
        region,
        [
            "logs",
    ...
    if log_groups_data is None:
        return make_check(
            "slow-queries",
            "RDS slow queries over last 24h",
            "ERROR",
            [],
            ["api_error=true"],
        )
    ...
    for log_group in slow_groups:
        db_label = (
            log_group.split("/")[4] if len(log_group.split("/")) > 4 else log_group
        )
        try:
    ...
    return make_check(
        "slow-queries", "RDS slow queries over last 24h", status, findings, evidence
    )
...
_TODO_TABLE_HEADER_RE = re.compile(
    r"^\| Priority \| Domain \| Task \| Jira \| Status \|"
)
...
def build_todo_tasks(
    checks: list[dict[str, Any]], run_date: str
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Weekly AWS health review (read-only)."
    )
    parser.add_argument(
        "--profile",
    ...
    parser.add_argument(
        "--output",
        default="",
        help="Output report file (default: auto-generated in reports/)",
    )
    parser.add_argument(
        "--no-todo-update",
        action="store_true",
...
def main(argv: list[str] | None = None) -> int:
    ...
    print(
        f"[aws-health-review] Starting review at {run_time.strftime('%Y-%m-%dT%H:%M:%SZ')}",
        file=sys.stderr,
    )
    ...
    for label, fn in checks:
        ...
        try:
            ...
            result = make_check(
                ...
                [
                    make_finding(
                        "INFO", f"Check raised an unexpected error: {exc}", "", "None"
                    )
                ],
                [f"error={exc}"],
            )
        ...
        print(
            f"[aws-health-review]   {label}: {status} ({finding_count} finding(s))",
            file=sys.stderr,
        )
    ...
    if not args.no_todo_update:
        ...
        if todo_path.exists():
            ...
            print(
                f"[aws-health-review] TODO.md updated: {added} new task(s) added.",
                file=sys.stderr,
            )
        else:
            print(
                f"[aws-health-review] TODO.md not found at {todo_path} — skipping update.",
                file=sys.stderr,
            )
